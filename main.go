...
func main() {
	...
	for scanner.Scan() {
		...
		for sayili != "yok" {
			...
			} else if strings.Contains(sayili, "upa") {
			...
			words = elemanSil(words, indeksim)
			sayili = icerirMi(words)
		}
		...
		eklenecek = fixQuotess(eklenecek)
...
func fixQuotess(text string) string {
	re := regexp.MustCompile(`'\s*(.*?)\s*'`)
	result := re.ReplaceAllString(text, "'$1'")
	return result
}

func elemanSil(arr []string, index int) []string {
	otherArr := []string{}

	for i := 0; i < len(arr); i++ {
		if i != index {
			otherArr = append(otherArr, arr[i])
		}
	}
